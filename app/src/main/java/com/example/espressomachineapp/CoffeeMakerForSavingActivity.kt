...
import kotlinx.android.synthetic.main.activity_coffee_maker.SelectedCoffeeText
import kotlinx.android.synthetic.main.activity_coffee_maker.doubleMeasureBtn
import kotlinx.android.synthetic.main.activity_coffee_maker.howManyShotsTitle
...
import kotlinx.android.synthetic.main.activity_coffee_maker.selectionsId
import kotlinx.android.synthetic.main.activity_coffee_maker.singleMeasureBtn
import kotlinx.android.synthetic.main.activity_coffee_maker_for_saving.*
import kotlinx.android.synthetic.main.alert_box.view.*
import java.util.*

class CoffeeMakerForSavingActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_coffee_maker_for_saving)
        SelectedCoffeeText.setText(loadDataString())
        ...
        saveCoffeeBtn.setOnClickListener {
            var OK: Boolean = true
            ...
            dialogLayout.okNameInputBtn.setOnClickListener {
                ...
                var dose = if (loadFlagData() == true) "Μονός" else "Single"
                var coffee = loadDataString()
                if (currentDose == doubleMeasureBtn.id) {
                    dose = if (loadFlagData() == true) "Διπλός" else "Double"
                }
                if (coffee != null) {
                    if (text.isEmpty()) {
                        OK = false
                    ...
                    if (currentPersons != null) {
                        for (person in currentPersons) {
                            if (person.toString()
                                    .equals(text + " " + coffee + " " + dose + " " + currentML)
                            ) {
                ...
                if (OK) {
                    if (coffee != null) {
                        insertItem(text, coffee, dose, "$currentML")
                    }
    ...
    private fun inputSpeech(CODE: Int) {
        if (CODE == 0) {      // if user saved his drink destroy speech
            return
        }
    ...
    private fun speakOut() {
        if (loadFlagData() == true) {
            ...
            tts!!.speak(
                "$title. Πείτε μου σας παρακαλώ τι θέλετε να αλλάξουμε; Οι επιλογές είναι. Δόσεις. Ποσότητα. Αποθήκευση. Αν θέλετε να πάτε στην προηγούμενη σελίδα, πείτε πίσω! Παρακαλώ, μιλήστε μετά τον ήχο!", //Hello. How could I serve you? Speak after the tone.
            ...
            tts!!.speak(
                "$title. Please tell me what you would like to change? Choices are shots. amount. Save. If you want to go back, say go back! Speak after the tone.", //Hello. How could I serve you? Speak after the tone.
                TextToSpeech.QUEUE_FLUSH,
                null,
                ""
            )
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == RQ_SPEECH_MENU && resultCode == Activity.RESULT_OK) {
            ...
            if (word.toUpperCase() == "ΠΊΣΩ" || word.toUpperCase() == "GO BACK") {
                ...
                startActivity(back)
                finish()
            } else if (word.toUpperCase() == "ΔΏΣΕΙΣ" || word.toUpperCase() == "SHOTS"
            ) {    // GOOGLE CANNOT CAPTURE Δόσεις!
                if (loadFlagData() == true) {
                    tts!!.speak(
                ...
                if (loadFlagData() == true) {
                    tts!!.speak(
                        "Οι επιλογές που κάνατε είναι.  $coffeeSelected .  $messageDose Δόση , $currentML εμέλ. Θέλετε να αποθηκευτεί το ρόφημα με αυτές τις επιλογές; Παρακαλώ μιλήστε μετά τον ήχο!", //Hello.
                        TextToSpeech.QUEUE_FLUSH,
                        null,
                        ""
                    )
                    ...
                    tts!!.speak(
                        "The choices you made are. $coffeeSelected , $messageDose Shot ,  $currentML ml. Do you want to save this drink? Speak after the ton!", //Hello.
                        TextToSpeech.QUEUE_FLUSH,
                        null,
                        ""
                    )
                ...
                tts!!.speak(
                    "Οι επιλογές είναι. Δόσεις. Ποσότητα. Αποθήκευση. Μιλήστε μετά τον ήχο!", //Hello.
                    TextToSpeech.QUEUE_FLUSH,
                    null,
                    ""
                )
                ...
                tts!!.speak(
                    "The choices are. Shots. Amount. Save. Please speak after the tone!", //Hello.
                    TextToSpeech.QUEUE_FLUSH,
                    null,
                    ""
                )
        ...
        if (requestCode == RQ_SPEECH_NAME_INPUT && resultCode == Activity.RESULT_OK) {
            ...
            if (coffee != null) {
                ...
                if (currentPersons != null) {
                    ...
                    for (person in currentPersons) {
                        if (person.toString()
                                .equals(word + " " + coffee + " " + dose + " " + currentML)
                        ) {
                            OK = false
                            break
                        }
            ...
            if (OK) {
                ...
                if (coffee != null) {
                    insertItem(word, coffee, dose, "$currentML")
                }
                val go_back = Intent(this, FavoritesActivity::class.java)
                startActivity(go_back)
            } else if (loadFlagData() != true) {
    ...
    private fun loadDataString(): String? {
        ...
        val savedString = sharedPreferences.getString("FAVOR", null)
        return savedString
